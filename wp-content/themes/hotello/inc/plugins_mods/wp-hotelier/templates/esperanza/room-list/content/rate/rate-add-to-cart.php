...
<?php if ($available_rooms > 0 && $is_available && apply_filters('hotelier_show_rate_add_to_cart_button', true, $checkin, $checkout, $variation)) : ?>
    ...
    <div class="add-to-cart-wrapper add-to-cart-wrapper--rate">
        ...
        <input type="hidden" name="add_to_cart_room[<?php echo esc_attr($key); ?>]"
               value="<?php echo esc_attr(apply_filters('hotelier_add_to_cart_room_id', $room->id, $room)); ?>"/>
        <input type="hidden" name="rate_id[<?php echo esc_attr($key); ?>]"
               value="<?php echo absint($variation->get_room_index()); ?>"/>

        <a href="#reserve-button"
           data-selected-text-singular="<?php echo esc_attr_x('room selected', 'book now button text: singular', 'hotello'); ?>"
           data-selected-text-plural="<?php echo esc_attr_x('rooms selected', 'book now button text: plural', 'hotello'); ?>"
           class="btn btn_third btn_icon-right btn_solid button button--add-to-cart">
            <?php esc_html_e('Book now', 'hotello'); ?>
