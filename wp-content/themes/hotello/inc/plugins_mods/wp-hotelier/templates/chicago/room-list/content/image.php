...
$gallery_images_ids = $room->get_gallery_attachment_ids();
if ($gallery_images_ids) {
    $images_qty += count($gallery_images_ids);
}
...
<div class="room__image room__image--listing">
    <?php if ($images_qty > 0) : ?>
        <div class="room__images-quantity">
            <span><?php echo wp_kses_post($images_qty); ?></span>
        </div>
    <?php endif; ?>
    ...
    if (has_post_thumbnail()) {
        ...
        $image = hotello_get_VC_post_img_safe($post->ID, '265x200');
